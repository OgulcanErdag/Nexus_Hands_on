...
- Launch a t3a.medium (Nexus needs 8 GB of RAM) EC2 instance using the Amazon Linux 2023 AMI with a security group allowing `SSH (22), Nexus Port (8081), 8082, and 8083` connections.
...
- Install Java:
Nexus and Maven are Java-based applications, so to run Nexus and Maven, we have to install Java on the server.
...
```bash
cd /opt
sudo wget -O nexus.tar.gz https://download.sonatype.com/nexus/3/nexus-3.90.2-06-linux-x86_64.tar.gz
ls
```

```bash
sudo tar xvz --keep-directory-symlink -f nexus.tar.gz
ls
...
```bash
sudo useradd --system --no-create-home --shell /sbin/nologin nexus
```
...
- Configure Nexus so that the ec2-user is going to be running the service. Edit the nexus.rc file with this `run_as_user="nexus"` content:
...
sudo systemctl enable --now nexus # enable and start comand together
sudo systemctl status nexus # If you fail, debug with the following command
```

* One of the most useful debugging commands when working with systemd services

```bash
sudo journalctl -u nexus -n 100 --no-pager # if you see "Not enough space" that means you need more Memory, be sure your ec2 is a medium machine.
```
...
- Open the Nexus service from the browser and click `Sign in` upper right of the page. A box will pop up.
Write `admin` for Username and paste the string that you copied from admin.password file for the password.
...
- Click the settings button after that `Repositories`. Then click `Create repository` and choose recipe: `maven2 (proxy)`.
...
- Once you see BUILD SUCCESS, you can see the components in the repository from the UI. Check the components inside the snapshot and the public repository. 
...
- Click the settings button after that `Repositories`. Then click `Create repository` and choose recipe: `maven2 (group)`.
...
- Init System: Init (short for initialization) is the first process started during the booting of the computer system. It is a daemon process that continues running until the system is shut down. It also controls services in the background. To start the Docker service, the init system should be informed.

```bash
sudo systemctl start docker
```

- Enable the Docker service so that it can restart automatically after reboots.
...
- Normally, the user needs to re-login into the bash shell for the group `docker` to be effective, but the `newgrp` command can be used to activate the `docker` group for `ec2-user`, not to re-login into the bash shell.
...
```bash
docker login -u admin -p admin 3.90.40.33:8082 # don't forget to change password.
docker pull 3.90.40.33:8082/alpine
docker image ls
```
...
```bash
docker login -u admin -p admin 3.90.40.33:8083
docker pull 3.90.40.33:8082/nginx
docker image ls
docker tag 3.90.40.33:8082/nginx 3.90.40.33:8083/myng
docker push 3.90.40.33:8083/myng
```
